...
const BatcavePanorama = ({ onNext, onProgress, isPaused }: BatcavePanoramaProps) => {
  ...
  useEffect(() => {
    ...
    let hasLoaded = false;
    let imgTexRef: THREE.Texture | null = null;
    let videoTexRef: THREE.VideoTexture | null = null;

    // Carica immagine statica come fallback immediato
    const loader = new THREE.TextureLoader();
    loader.load("/assets/textures/BatCaverna360_BatComputerArea.png", (imgTex) => {
    ...
    const onCanPlay = () => {
      ...
      videoTexRef = tex;
      // Dispose the static image texture since we now have the video
      if (imgTexRef) {
        imgTexRef.dispose();
        imgTexRef = null;
      }
    ...
    return () => {
      ...
      // CRITICAL: dispose textures to free GPU memory and prevent WebGL Context Lost
      if (videoTexRef) {
        videoTexRef.dispose();
        videoTexRef = null;
      }
  ...
  const handleCardSuccess = (id: number) => {
    ...
    if (newCompleted.length === RIDDLES.length) {
      ...
      const nextVid = document.createElement("video");
      nextVid.src = "/assets/videos/BatCaverna_PassaggioBatComputerAArmeria.mp4";

...
const ArmeriaPanorama = ({ onNext, onProgress, baseCompleted, isPaused }: ArmeriaPanoramaProps) => {
  ...
  // Delay Canvas mount slightly to give GPU time to release previous WebGL context
  ...
  useEffect(() => {
    ...
    const loadWithFallback = (idx: number) => {
      if (idx >= urls.length) {
        if (mounted) {
          console.error("All texture fallbacks failed for ArmeriaArea");
          setIsEntering(false);
        }
        return;
      }
    ...
    // Emergency safety: always exit loading after 6 seconds
  ...
  const handleCardSuccess = useCallback(() => {
    ...
    nextVid.src = "/assets/videos/BatCaverna_PassaggioArmeriaABatMobile.mp4";
      ...
      {texture && canvasReady && (
        <Canvas gl={{ antialias: true, toneMapping: THREE.NoToneMapping, powerPreference: "high-performance" }}>
          ...
          <Scene
            ...
            onCardClick={() => setActiveCardId(ARMERIA_RIDDLE.id)}

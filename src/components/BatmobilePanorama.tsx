...
const BatmobilePanorama = ({ onNext, onProgress, baseCompleted, isPaused }: BatmobilePanoramaProps) => {
  ...
  useEffect(() => {
    ...
    const urls = ["/BatCaverna360_BatMobileArea.jpg", "/BatCaverna360_BatMobileArea.png"];
    ...
    <div className="fixed inset-0 w-full h-full bg-black z-40 overflow-hidden">
      ...
      {texture && (
        <Canvas gl={{ antialias: true, toneMapping: THREE.NoToneMapping }}>
          ...
          {spawnPositions && <Scene texture={texture} spawnPositions={spawnPositions} activeCardId={activeCardId} completedIds={completedIds} onCardClick={(id: number) => setActiveCardId(id)} onCloseCard={() => setActiveCardId(null)} onCardSuccess={handleCardSuccess} isPaused={isPaused} />}
        </Canvas>
      )}
